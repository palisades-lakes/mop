...
;(defn tmpdir [] (System/getProperty "java.io.tmpdir"))
;
;(defn tmpname []
...
(defn make-shader [source shader-type]
      (let [shader (GL20/glCreateShader shader-type)]
           (^[int CharSequence/1] GL20/glShaderSource shader source)
...
(defn setup-vao [vertices indices]
      (let [vao (GL30/glGenVertexArrays)
           ...
           (^[int ByteBuffer int] GL15/glBufferData GL15/GL_ARRAY_BUFFER (make-float-buffer vertices)
                                                    GL15/GL_STATIC_DRAW)
           (GL15/glBindBuffer GL15/GL_ELEMENT_ARRAY_BUFFER ibo)
           (^[int ByteBuffer int] GL15/glBufferData GL15/GL_ELEMENT_ARRAY_BUFFER (make-int-buffer indices)
                                                    GL15/GL_STATIC_DRAW)
           {:vao vao :vbo vbo :ibo ibo}))

(defn teardown-vao [{:keys [vao vbo ibo]}]
      (GL15/glBindBuffer GL15/GL_ELEMENT_ARRAY_BUFFER 0)
      (^[IntBuffer] GL15/glDeleteBuffers ibo)
      (GL15/glBindBuffer GL15/GL_ARRAY_BUFFER 0)
      (^[IntBuffer] GL15/glDeleteBuffers vbo)
      (GL30/glBindVertexArray 0)
      (^[IntBuffer] GL15/glDeleteBuffers vao))
